...
[![Stack](https://img.shields.io/badge/Tech-WebAudio%20%7C%20TailwindCSS%20%7C%20Python%20%7C%20C%23-informational)](#tech-stack)
[![Status](https://img.shields.io/badge/Environment-Offline--Ready%20Single--File-emerald)](#deployment--usage)
...
|x[n]| + \alpha_{\text{att}} \cdot (\text{env}[n-1] - |x[n]|), & \text{if } |x[n]| > \text{env}[n-1] \\ 
|x[n]| + \alpha_{\text{rel}} \cdot (\text{env}[n-1] - |x[n]|), & \text{if } |x[n]| \le \text{env}[n-1] 
\end{cases}$$

Gain reduction applied with a lookahead buffer delay ($D \approx 5\text{ ms}$):

$$g[n] = \begin{cases} 
1.0, & \text{if } \text{env}[n] \le \text{Threshold} \\ 
\frac{\text{Threshold}}{\text{env}[n]}, & \text{if } \text{env}[n] > \text{Threshold} 
\end{cases}$$
...
$$f(x) = \left| \left( (x + 1) \bmod 4 \right) - 2 \right| - 1$$
...
- **DAWs:**
  - [FL Studio](https://www.image-line.com/fl-studio/): High-speed step sequencing, microtonal piano roll slides, Sytrus/Harmor engines.
  - [Ableton Live](https://www.ableton.com/): Real-time audio warping (Complex Pro), session view clip sequencing, Max for Live (M4L) visual programming.
  - [Reaper](https://www.reaper.fm/): Lightweight digital workstation with native JSFX scriptable audio DSP.
...
- **Modern Software Synths:**
  ...
  - [Surge XT](https://surge-synthesizer.github.io/): Open-source hybrid synthesizer.
...
- [CSCore](https://github.com/filoe/cscore) - Advanced .NET audio processing library.
